...
#Sampler using Langevin Dynamics - Gradient descent with brownian motion
class LangevinMonteCarlo:
    def __init__(self, x0, target, transition=NormalProposal(), tau=.1, burn=1000, MALA=False):
    ...
    #Define langevin dynamics step (Euler-Muruyama method)
    #Implements: x_{k+1} = x_k + tau * grad(target(x_k)) + noise ~ N(mu,sigma)
    def step(self, x):
        x_brownian = self.transition.sample(x)
        grad_step = self.tau * self.target.grad(x)

        return x_brownian + grad_step
    ...
    def __call__(self, iterations, all=False):
        ...
        for i in range(iterations):
            ...
            #Run sampling depending on MALA usage
            if not self.MALA:
                if self.burnt:
                    if self.counter == 0: 
                        self.samples = []
        ...
        if not self.MALA:
            if all:
                return np.array(self.burnsamples), np.array(self.samples)
            else:
